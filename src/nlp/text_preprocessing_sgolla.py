...
input_path: Path = DATA_PATH / "text_data_sgolla.txt"
...
# Technical modification: Show only tokens that appear more than once (rare terms removed).
freq_df_filtered: pl.DataFrame = freq_df.filter(pl.col("len") > 1)

print("Top 20 most frequent cleaned tokens (count > 1):")
print(freq_df_filtered.head(20))

# Keep top 10 of the filtered tokens for plotting.
top_df: pl.DataFrame = freq_df_filtered.head(10)
...
# ============================================================
# Section 12. Technical Modification: Lexical Diversity
# ============================================================

# Lexical diversity is the ratio of unique tokens to total tokens.
# A higher score indicates a more diverse or "rich" vocabulary.

unique_tokens_count: int = freq_df.height
lexical_diversity: float = (
    unique_tokens_count / count_of_clean_tokens if count_of_clean_tokens > 0 else 0
)

print("\nTechnical Modification: Lexical Diversity Analysis")
print(f"- Total Cleaned Tokens: {count_of_clean_tokens:,}")
print(f"- Unique Tokens (Vocabulary Size): {unique_tokens_count:,}")
print(f"- Lexical Diversity Score: {lexical_diversity:.4f}")

# Build a Pie Chart for Lexical Diversity
labels = ["Unique Tokens", "Repeated Tokens"]
repeated_count = count_of_clean_tokens - unique_tokens_count
sizes = [unique_tokens_count, repeated_count]
